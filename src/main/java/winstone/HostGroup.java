...
//        this.args = args;
        ...
        // If host mode
        if (hostDirName == null) {
            initHost(webappsDirName, DEFAULT_HOSTNAME, cluster, commonLibCL, 
        ...
        // Otherwise multi-webapp mode
            ...
            initMultiHostDir(hostDirName, cluster, commonLibCL, 
                    commonLibCLPaths, args);
    ...
    protected void initHost(String webappsDirName, String hostname, Cluster cluster, 
            ClassLoader commonLibCL, File commonLibCLPaths[], Map args) throws IOException {
        Logger.log(Logger.DEBUG, Launcher.RESOURCES, "HostGroup.DeployingHost", hostname);
        HostConfiguration config = new HostConfiguration(hostname, cluster, commonLibCL, 
                ...
                if (children[n].isDirectory()) {
                    if (!this.hostConfigs.containsKey(childName)) {
                        initHost(children[n].getCanonicalPath(), childName, cluster, 
                                commonLibCL, commonLibCLPaths, args);
                    }
                }
